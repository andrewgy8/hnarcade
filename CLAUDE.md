...
- `src/components/NewsletterBanner/` — dismissible newsletter signup banner component
...
- `static/img/favicon.svg` — site favicon (arcade cabinet in HN orange #ff6600)
- `static/img/logo.svg` — site logo (default Docusaurus dinosaur)
- `static/img/newsletter-icon.png` — newsletter icon (currently unused)
...
The site has a newsletter signup page at `/newsletter` that uses [Buttondown](https://buttondown.email) for email collection. Subscribers receive weekly curated indie games from Hacker News. The signup form posts directly to Buttondown's embed API at `https://buttondown.email/api/emails/embed-subscribe/andrewgy8`.

Newsletter integration points:
- Newsletter page: `src/pages/newsletter.tsx`
- Dismissible banner: `src/components/NewsletterBanner/` (appears on first visit, uses localStorage to track dismissal)
- Archive link: `https://buttondown.com/andrewgy8/archive`

## Visual Assets

**Favicon:** The site uses `static/img/favicon.svg` as its favicon, featuring a custom-designed arcade cabinet icon in HN orange (#ff6600). The design includes:
- Orange cabinet body with screen, joystick, and buttons
- Green CRT-style screen with glare effect
- Red joystick knob and arcade buttons
- Yellow and blue secondary buttons
- Coin slot detail
- Cabinet legs

The favicon is configured in `docusaurus.config.ts` and provides a distinctive retro gaming aesthetic that reinforces the arcade theme.

**Logo:** The default Docusaurus dinosaur logo (`static/img/logo.svg`) is used but could be replaced with a custom logo in the future.

**Newsletter Icon:** A joystick icon at `static/img/newsletter-icon.png` complements the arcade theme with HN orange (#ff6600) styling. This icon is currently unused in the codebase but available for newsletter-related UI elements.
