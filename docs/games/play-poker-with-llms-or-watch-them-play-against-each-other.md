---
title: "llmholdem.com"
tags: [browser, free]
description: "Show HN: Play poker with LLMs, or watch them play against each other. Discovered via HN scraper."
sidebar_position: 5
---

# Play poker with LLMs, or watch them play against each other

Show HN: Play poker with LLMs, or watch them play against each other. Discovered via HN scraper.

| | |
|---|---|
| **Author** | [projectyang](https://news.ycombinator.com/user?id=projectyang) |
| **Play** | [llmholdem.com](https://llmholdem.com/) |
| **HN Thread** | [Show HN: Play poker with LLMs, or watch them play against each other](https://news.ycombinator.com/item?id=46569061) |

## About

Show HN: Play poker with LLMs, or watch them play against each other. Discovered via HN scraper.
